...
export function distanceBetweenPoints(_objAx, _objAy, _objBx, _objBy) {
    return Math.sqrt(Math.pow(_objAx - _objBx, 2) + Math.pow(_objAy - _objBy, 2))
}
export function magnitudeOfVec(x, y) {
    return Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2))
}
export function angleBetweenPoints(_p1x, _p1y, _p2x, _p2y) {
    return Math.atan2(_p2y - _p1y, _p2x - _p1x)
}

export function normalizeVector(_x, _y) {
...
export function roundToX(num, dec) {
...
export function getVertices(_obj, _data_type, _padding) {
    //_data_type can be "obj" or "arr", _padding is true/false

    var pad_x = 1.0
    var pad_y = 1.0
    if (_padding === true) {
        //if true, we should calculate with padding
    ...
    if (_obj.type === 'rectangle') {
        ...
        if (_obj.rotation !== 0) {
            var BRx =
                _obj.x +
                _obj.width * pad_x / 2 * Math.cos(_obj.rotation) -
                _obj.height * pad_y / 2 * Math.sin(_obj.rotation)
            var BRy =
                _obj.y +
                _obj.height * pad_y / 2 * Math.cos(_obj.rotation) +
                _obj.width * pad_x / 2 * Math.sin(_obj.rotation)
            var BLx =
                _obj.x -
                _obj.width * pad_x / 2 * Math.cos(_obj.rotation) -
                _obj.height * pad_y / 2 * Math.sin(_obj.rotation)
...
export function lineIntersect2(
    line1StartX,
    line1StartY,
    line1EndX,
    line1EndY,
...
export function getRate(end_time, duration, easing) {
    var current_time = Date.now()
    easing = easing || 'linear'
    var remaining_time = end_time - current_time
    var easingEq = easings[easing]
